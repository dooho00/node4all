    ...
    def __init__(self,
        ...
        if num_layers == 1:
            ...
            for l in range(1, num_layers - 1):
                ...
                self.gat_layers.append(GATConv(
                    hidden_dim * nhead, hidden_dim, nhead,
                    feat_drop, attn_drop, negative_slope, residual, activation, norm=norm, concat_out=concat_out))
            # output projection
            self.gat_layers.append(GATConv(
                hidden_dim * nhead, out_dim, nhead_out,
...
class GATConv(nn.Module):
    ...
    def forward(self, graph, feat, get_attention=False):
        with graph.local_scope():
            if not self._allow_zero_in_degree:
                if (graph.in_degrees() == 0).any():
                    raise RuntimeError('There are 0-in-degree nodes in the graph, '
                                   ...
                                   'suppress the check and let the code run. This '
                                   'is not recommended unless you know the '
                                   'input graph is safe to run on.')
            ...
            attention = self.attn_drop(edge_softmax(graph, e))

            graph.edata['a'] = attention
