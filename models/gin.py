...
class GIN(nn.Module):
    def __init__(self,
        ...
        if num_layers == 1:
            apply_func = MLP(2, in_dim, hidden_dim, out_dim, activation=activation, norm=norm)
            if last_norm:
            ...
            self.layers.append(GINConv(
                in_dim, 
                hidden_dim, 
                ApplyNodeFunc(MLP(2, in_dim, hidden_dim, hidden_dim, activation=activation, norm=norm), activation=activation, norm=norm), 
                init_eps=0,
                learn_eps=learn_eps,
                residual=residual)
                )
            ...
            for l in range(1, num_layers - 1):
                ...
                self.layers.append(GINConv(
                    ...
                    ApplyNodeFunc(MLP(2, hidden_dim, hidden_dim, hidden_dim, activation=activation, norm=norm), activation=activation, norm=norm), 
                    init_eps=0,
                    learn_eps=learn_eps,
                    residual=residual)
                )
            # output projection
            apply_func = MLP(2, hidden_dim, hidden_dim, out_dim, activation=activation, norm=norm)
            if last_norm:
                apply_func = ApplyNodeFunc(apply_func, activation=activation, norm=norm)

            self.layers.append(GINConv(hidden_dim, out_dim, apply_func, init_eps=0, learn_eps=learn_eps, residual=last_residual))
    ...
    def __init__(self, num_layers, input_dim, hidden_dim, output_dim, activation="relu", norm="batchnorm"):
        ...
        if num_layers < 1:
            ...
            for layer in range(num_layers - 1):
                self.norms.append(norm(hidden_dim))
